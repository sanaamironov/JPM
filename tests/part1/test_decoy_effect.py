...
def _simulate_decoy_data(N_per_type=300):
    """
    Two choice sets over 3 items {0,1,2}:
        A = {0,1}: P(0|A)=0.5, P(1|A)=0.5
        B = {0,1,2} with decoy: P(0|B)=0.7, P(1|B)=0.2, P(2|B)=0.1
    """
    J = 3
...
class TestDecoyEffect(unittest.TestCase):
    def test_decoy_effect_learned(self):
        ...
        model.fit(ds, epochs=40, verbose=0)

        avail_eval = tf.convert_to_tensor(np.stack([mask_A, mask_B]), dtype=tf.float32)
        item_ids_eval = tf.tile(tf.range(3)[tf.newaxis, :], [2, 1])
        out = model({"available": avail_eval, "item_ids": item_ids_eval}, training=False)
        probs = tf.exp(out["log_probs"]).numpy()

        # DeepHalo should learn P(0|B) > P(0|A) — the decoy effect.
        self.assertGreater(probs[1][0], probs[0][0] + 0.05)

...
Outputs:
    results/table1_predictions.csv
    results/heatmap_target.png
    results/heatmap_predicted.png
-------------------------------------------------------------
"""
...
# ---------------------------------------------------------------------
# Ensure part_1/ is importable no matter where we run from
# ---------------------------------------------------------------------
...
def build_sampled_dataset(rows, draws_per_row: int = 1000, seed: int = 0):
    """
...
def make_heatmap(matrix: np.ndarray, title: str, path: Path):
...
def main():
    ...
    out = model({"available": eval_available, "item_ids": eval_item_ids}, training=False)
    probs = tf.exp(out["log_probs"]).numpy()

    # ----------------------------------------------------
    # 6. Save outputs under part_1/results/
    # ----------------------------------------------------
    out_dir = results_dir()
    fig_dir = figures_dir()
    csv_path = out_dir / "table1_predictions.csv"
    ...
    target_path = fig_dir / "heatmap_target.png"
    pred_path = fig_dir / "heatmap_predicted.png"

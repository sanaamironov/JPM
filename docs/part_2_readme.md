...
## Codebase structure and separation of concerns

Part 2 intentionally separates **replication**, **framework implementation**, and **model extension**:

- **`replication_lu25/` (standalone replication)**  
  A self-contained Python implementation of Lu & Shimizu (2025), Section 4.  
  This code:
  - does **not** depend on `choice-learn`,
  - follows a classical econometric structure (BLP, IVs, shrinkage),
  - is used exclusively to replicate the paper’s Monte Carlo results.

- **`choice_learn_extension/` (Lu-style estimation in choice-learn)**  
  A TensorFlow / TensorFlow Probability implementation of Lu-style sparse
  market–product shocks written to match the `choice-learn` API.
  This satisfies the requirement to implement Lu (2025)’s model **inside**
  a modern ML framework.

- **`extension_deephalo/` (Zhang + Lu hybrid model)**  
  This folder contains the **original extension** that combines:
  - Zhang et al. (2025)’s DeepHalo context-dependent neural utility model, and
  - Lu (2025)’s sparse market–product shock assumption.

  This hybrid model directly addresses Part 2(d):  
  *modifying the deep context-dependent choice model under Lu-style assumptions
  and validating the modification via simulation.*

These components are deliberately kept separate to ensure clarity,
reproducibility, and clean attribution of results.
